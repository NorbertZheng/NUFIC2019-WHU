    ...
    for file in range(files_num):
        ...
        for i in range(10):
            if(i==0):#如果是第0段数字，开头的50ms去掉，相当于这里面是5个采样点
                number_segs[i][0:int(data_seg[i][0][1])-int(data_seg[i][0][0])]=df[['ADGx','ADGy','ADGz']][int(data_seg[i][0][0]):int(data_seg[i][0][1])]
            ...
            if(i==9):#最后5个采样点不要
                quiet_segs[i][0:(int(data_seg[i][1][1])-int(data_seg[i][1][0]))]=df[['ADGx','ADGy','ADGz']][int(data_seg[i][1][0]):int(data_seg[i][1][1])]
            else:
                quiet_segs[i][0:(int(data_seg[i][1][1])-int(data_seg[i][1][0]))]=df[['ADGx','ADGy','ADGz']][int(data_seg[i][1][0]):int(data_seg[i][1][1])]
